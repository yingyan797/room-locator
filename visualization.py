        ...
        if node is not None:
            ...
            if level < 0:
                ...
                if node.attr:
                    ...
                    plt.text(node.x, node.y, f" pred: {node.val}", ha='center',
                             bbox={"boxstyle": "round", "pad": 0.4, "facecolor": "w", "edgecolor": "k"},
                             fontsize="small")
                self.plot_tree(node.left, level - 1, position - 2 ** level / 15, (node.x, node.y))
                self.plot_tree(node.right, level - 1, position + 2 ** level / 15, (node.x, node.y))
